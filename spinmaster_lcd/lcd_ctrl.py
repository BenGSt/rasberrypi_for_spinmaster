...
def LCD_plot(text_arr):
    ...
    if(text_arr_len == 1):    
        LCD_plot_line(text_arr[0], 1)       # Write line of text to 1 line of display                
    elif(text_arr_len == 2):    
        LCD_plot_line(text_arr[0], 1)       # Write line of text to 1 line of display 
        LCD_plot_line(text_arr[1], 2)       # Write line of text to 2 line of display   
    else:
        for i in range(text_arr_len * 2):
            LCD_plot_line(text_arr[i % text_arr_len], 1)
            LCD_plot_line(text_arr[(i + 1) % text_arr_len], 2)
            sleep(1)                         # delay of 1 second
            lcd_clear()
...
def write_line_to_file(file_address, line_num, lst):
...
if __name__ == "__main__":
    ...
    if(args.plot != ''):
        lst = list()
        new_lst = write_line_to_file('memory', args.line, lst)
        LCD_plot(new_lst)
